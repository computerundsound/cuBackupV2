...
define('CU_BACKUP_VERSION', '3.0.0');
...
$actions = [
    ...
    'zip'                    => [
        'text'      => 'Create a zip-file from whole directory and all subdirectories - <strong>no</strong>
...
class Backup
{
     ...
     */
    public function tarGz($tarGzFileOnServer, array $excludedDirs)
    {

        $this->runExec("tar -vczf $tarGzFileOnServer ./. -x " . $fileNames);
     ...
     */
    public function zipExec($dir, $zipFileOnServer, $excludedDirs)
    {

        /* zip - exe */

        $fileNamesExcluded = implode('/\\* ', $excludedDirs) . '/\\*';

        $response = $this->runExec("zip -r $zipFileOnServer $dir -x $fileNamesExcluded");
...
try {
    ...
    switch ($action) {
        ...
        case 'tar':
            ...
            Output::addParagraphHeading('Action: ' . $action);
            Output::add('TARRRRRRR');

            $cuBackup->tarGz($tarGzFileOnServer, $excludedDirs);
        ...
        case 'zip':
            ...
            if ($actionModus[$action] === 'php') {
                ...
                Output::add('PHP....');
                $cuBackup->zipExec($dir, $zipFileOnServer, $excludedDirs);
            }
                ...
                foreach ($backupDirs as &$backupDir) {
                    ...
                    if (realpath($dir)) {
                        ...
                        if ($actionModus[$action] === 'php') {
                            ...
                            $cuBackup->zipExec($dir, $zipFileOnServerFullPath, $excludedDirs);
                            Output::addStrong("Files: ");
                            Output::addMultiLines($allFiles);
                        }
                    } else {
        ...
        <div class="col-md-12">
            ...
            <p>
                <button class="btn btn-primary"
                        type="button"
                        data-toggle="collapse"
                        data-target="#collapseDBBackupInfo"
                        aria-expanded="false"
                        aria-controls="collapseDBBackupInfo">
                    Show helping commands
                </button>
            </p>

            <div class="alert alert-info collapse" id="collapseDBBackupInfo">

                <h4>Zip via SSH (example)</h4>
                <p>This will zip all files recursiv except all directories after the -x flag to the file
                   cuBackup.zip</p>
                <code>
                    <pre>zip -r cuBackup.zip * -x ./cache/**\* ./logfiles/**\* ./logs/**\* ./templates_c/**\* ./uploads/**\* ./export/invoice/**\* ./images/product_images/**\*</pre>
                </code>

                <h4>Restore Database from sql-file</h4>
                <p>This is how you can update your local DB with the created DB File from this tool</p>
                <code>
                    <pre>mysql -u root -p dbNameOn < cuBackup.sql</pre>
                </code>
    ...
    <div class="row">
        <div class="col">
            ...
            <div class="row">
                <div class="col">
                    <div class="alert alert-info">
                        <p>Path to this file: <?php echo __DIR__; ?></p>
                    </div>
                </div>
            </div>
